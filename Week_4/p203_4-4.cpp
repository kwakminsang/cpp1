...
// 게임 상태 저장 함수
void saveGame(const vector<vector<int>>& puzzle, int moveCount, system_clock::time_point startTime, system_clock::time_point endTime, const string& filename) {
    ...
    // 현재 시간을 구하기
    auto durationStart = duration_cast<seconds>(startTime.time_since_epoch()).count();
    auto durationEnd = duration_cast<seconds>(endTime.time_since_epoch()).count();

    // 게임 상태 저장
    outFile << "퍼즐 크기: " << puzzle.size() << endl;
    outFile << "이동 횟수: " << moveCount << endl;
    outFile << "게임 시작 시간: " << durationStart << "초" << endl;
    outFile << "게임 종료 시간: " << durationEnd << "초" << endl;
...
// 게임 상태 로드 함수
vector<vector<int>> loadGame(const string& filename, int& moveCount, system_clock::time_point& startTime, system_clock::time_point& endTime) {
    ...
    inFile >> line >> startEpoch; // "게임 시작 시간" 읽기
    inFile >> line >> endEpoch;   // "게임 종료 시간" 읽기

    startTime = system_clock::from_time_t(startEpoch); // 시작 시간을 시스템 시간으로 변환
    endTime = system_clock::from_time_t(endEpoch);     // 종료 시간을 시스템 시간으로 변환
...
int main() {
    ...
    auto endTime = startTime; // 종료 시간 초기화

    string direction;
    while (true) {
        cout << "\n움직일 방향을 입력하세요 (up, down, left, right, exit, save, load): ";
        cin >> direction;

        if (direction == "exit") {
            endTime = system_clock::now(); // 게임 종료 시간 기록
            saveGame(puzzle, moveCount, startTime, endTime, "game_save.txt");
            break; // 게임 종료
        }
        ...
        if (direction == "load") {
            endTime = system_clock::now(); // 게임 종료 시간 기록
            puzzle = loadGame("game_save.txt", moveCount, startTime, endTime); // 게임 상태 로드
            cout << "게임이 로드되었습니다." << endl;
            printPuzzle(puzzle);
            continue;
        }
